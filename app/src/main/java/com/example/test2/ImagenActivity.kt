...
import com.example.test2.databinding.ActivityImagenBinding
...
class ImagenActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        drawerLayout.keepScreenOn = true
        ...
        permissionsLauncher = registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) { permissions ->
            ...
            if(readPermissionGranted) {
                ...
                alertDialogBuilder.setPositiveButton(android.R.string.ok) { _, _ ->
                    val intent = Intent(this@ImagenActivity, Settings.ACTION_APPLICATION_SETTINGS::class.java)
        ...
        buttonDelete.setOnClickListener { lifecycleScope.launch {
            intent.getStringExtra("Id")?.let { it1 ->
        ...
        return withContext(Dispatchers.IO) {
            ...
            contentResolver.query(
                ...
                while(cursor.moveToNext()) {
                    ...
                    photos.add(SharedStoragePhoto(id = id, name = displayName, width = width, height = height, contentUri = contentUri, fotoid = 0))
                }
                photos.toList()
            } ?: listOf()
        }
    ...
    private fun decodeSampledBitmapFromUri(uri: Uri, reqWidth: Int, reqHeight: Int): Bitmap? {
        ...
        val options = BitmapFactory.Options().apply {
            ...
            this.inSampleSize = inSampleSize  // Use "this" to refer to the Options object
        }

        return fileDescriptor?.let {
    ...
    private suspend fun deletePhotoFromExternalStorage(photoUri: Uri, fotoid: String) {
        if (isFotoDeleted) {
            withContext(Dispatchers.IO) {
                try {
                    contentResolver.delete(photoUri, null, null)
                    sqliteHelper.borrarFoto(fotoid)
